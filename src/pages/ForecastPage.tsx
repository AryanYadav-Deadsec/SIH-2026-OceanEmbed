      ...
      <PageContainer>
        <PageHeader
          ...
          badge={
            <div className="flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-cyan-500/15 border border-cyan-500/30 text-cyan-300 text-[11px] font-mono">
              ...
              CONVGRU 7-DAY PROJECTION
            </div>
          }
          icon={<Calendar size={18} className="text-cyan-400" />}
          title="7-Day Subsurface Temperature Forecast"
          subtitle={`Production ConvGRU neural model forecast · 15 standard depth layers from ${predictionDates[0]} to ${predictionDates[predictionDates.length - 1]}`}
          actions={
            <button
              onClick={loadBackendData}
              className="btn-glass"
        ...
        <div className="flex flex-wrap gap-3 mb-8">
          ...
          {[
            ...
            {
              label: 'Backend',
              value: getBackendUrl().replace(/^https?:\/\//, '') || 'Auto-Proxy',
            },
          ].map(
            ({
              label,
